# Tags

Browse all posts by topic tags. Click on a tag to see related posts (via search).

<div class="tags-container">

## Architecture & Design

**Posts:** 6
**Topics:** Microservices, API Gateway, BFF patterns, Event-Driven Architecture, system design

Related Posts:
- [Migrate Monolithic to Microservice](#/posts/migrate-monolithic-to-microservice.md)
- [From Fragmented UIs to One PWA](#/posts/api-gateway-pwa-microservices.md)
- [BFF in Goalixa](#/posts/bff-goalixa.md)
- [Migrate to PWA Successfully](#/posts/migrate-to-pwa-success-fully.md)
- [Future Plan: Event-Driven Architecture with NATS](#/posts/event-driven-architecture-nats.md)
- [Removing UI from Legacy Services](#/posts/removing-ui-from-legacy-auth-redirection.md)

---

## DevOps & Infrastructure

**Posts:** 3
**Topics:** Kubernetes, monitoring, staging environments, Kustomize, reliability

Related Posts:
- [Monitoring Stack Setup: Prometheus, Grafana, Alertmanager](#/posts/monitoring-stack-prometheus-grafana-alertmanager.md)
- [Why Goalixa Needs a Staging Environment](#/posts/devops/why-staging-with-kustomize.md)
- [API Gateway and One PWA](#/posts/api-gateway-pwa-microservices.md)

---

## GitOps & Deployment

**Posts:** 2
**Topics:** ArgoCD, GitOps, deployment automation, sync strategies, Harbor

Related Posts:
- [ArgoCD Setup: First Step](#/posts/gitops/argocd-first-step.md) (Part 1)
- [ArgoCD Applications and Sync Strategy](#/posts/gitops/argocd-applications-sync-strategy.md) (Part 2)

---

## Performance & Latency

**Posts:** 2
**Topics:** Latency optimization, performance engineering, network optimization

Related Posts:
- [Latency Taught Me Better Software Engineering](#/posts/software-engineering/latency-taught-me-better-software-engineering.md)   
- [Incident Report: PWA Path Latency](#/posts/incident-reports/pwa-path-latency-incident.md)

---

## Software Engineering

**Posts:** 3
**Topics:** AI-assisted development, engineering practices, development workflow, Claude

Related Posts:
- [Latency Taught Me Better Software Engineering](#/posts/software-engineering/latency-taught-me-better-software-engineering.md)   
- [Using Claude to Speed Up Development](#/posts/software-engineering/using-claude-for-goalixa.md)
- [Why Goalixa Is AI-Developed](#/posts/why-goalixa-is-ai-developed.md)

---

## Incident Reports

**Posts:** 2
**Topics:** Production incidents, post-mortems, root cause analysis, lessons learned

Related Posts:
- [PWA Path Change: High Latency](#/posts/incident-reports/pwa-path-latency-incident.md)
- [Removing UI from Legacy Auth](#/posts/removing-ui-from-legacy-auth-redirection.md)

---

## Personal Journey

**Posts:** 3
**Topics:** Wartime challenges, Iran, internet restrictions, learning, personal story

Related Posts:
- [Survive in War Time](#/posts/goalixa-story/survive-in-war-time.md)   
- [Learning in Wartime: Preparing Infrastructure](#/posts/goalixa-story/learning-in-wartime-kubernetes-plan.md)
- [Survive in War Time](#/posts/goalixa-story/survive-in-war-time.md)

---

## Authentication & Security

**Posts:** 1
**Topics:** JWT, tokens, auth flow, security, session management

Related Posts:
- [Removing UI from Legacy Services: Rebuilding Auth Flow](#/posts/removing-ui-from-legacy-auth-redirection.md)

---

## Monitoring & Observability

**Posts:** 1
**Topics:** Prometheus, Grafana, Alertmanager, metrics, dashboards, alerts

Related Posts:
- [Monitoring Stack Setup](#/posts/monitoring-stack-prometheus-grafana-alertmanager.md)

---

## AI & Development Tools

**Posts:** 2
**Topics:** Claude, AI-assisted coding, development workflow, productivity

Related Posts:
- [Using Claude to Speed Up Development](#/posts/software-engineering/using-claude-for-goalixa.md)
- [Why Goalixa Is AI-Developed](#/posts/why-goalixa-is-ai-developed.md)

---

## Future Planning

**Posts:** 1
**Topics:** Roadmap, architecture planning, NATS, event-driven design

Related Posts:
- [Future Plan: Event-Driven Architecture with NATS](#/posts/event-driven-architecture-nats.md)

</div>

---

## Popular Tags Summary

| Tag | Post Count | Category |
|-----|-----------|----------|
| architecture | 6 | Architecture and Platform |
| software-engineering | 3 | Software Engineering |
| devops | 3 | DevOps |
| gitops | 2 | GitOps |
| performance | 2 | Software Engineering |
| incident | 2 | Incident Reports |
| ai | 2 | Software Engineering |
| personal | 3 | Goalixa Story |

---

**Note:** This blog uses Docsify's search functionality. To find all posts with a specific tag, use the search box and type the tag name.
